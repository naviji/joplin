...
export default class SearchEngine {
	...
	parseQuery(query: string) {
		...
		// TODO : Make this part typed and extract to a seperate function
		...
		//
		// I haven't tested code with wildcards clearly.
		// Why does the string get replaced by an object in case of asterisks?
		// Ans: Probably used in the hightlighting part of the code
		//

		// const convertWildcardsToRegex = (terms) => {
		// 	let result = []
		...
		// 		if (term === '*') continue;
		// 		if (term.indexOf('*') >= 0) {
		// 			result.push({ type: 'regex', value: term, scriptType: scriptType(term), valueRegex: this.queryTermToRegex(term) });
		// 		} else {
		// 			result.push({ type: 'text', value: term, scriptType: scriptType(term) });
		// 		}
		// 	}
		// 	return result;
		// }

		// terms._ = convertWildcardsToRegex(terms._);
		...
		// TODO: Change the code/variable names to make this comment unnecessary
		// Maybe rename "terms" to "termsToHighlight" and "allTerms" to "filterTerms" and "normalizedFilterTerms"
		...
		const termCount = terms._.length + terms.body.length + terms.title.length;
		const keys = [];
		if (terms._.length) keys.push('_');
		if (terms.body.length) keys.push('body');
		if (terms.title.length) keys.push('title');
	...
	async basicSearch(query: string) {
		...
		if (parsedQuery.terms._.length) {
			const term = parsedQuery.terms._.map((x: any) => x.value).join(' ');
			searchOptions.anywherePattern = `*${term}*`;
		}

		if (parsedQuery.terms.title.length) {
			const term = parsedQuery.terms.title.map((x: any) => x.value).join(' ');
			searchOptions.titlePattern = `*${term}*`;
		}

		if (parsedQuery.terms.body.length) {
			const term = parsedQuery.terms.body.map((x: any) => x.value).join(' ');
